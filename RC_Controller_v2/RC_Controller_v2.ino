// Uses an ItsyBitsy 23u4 with two analog joysticks (right and left). Sends to nRF24L01 on the car being controlled
...
int joystick1ButtonPin = 10;
int joystick2ButtonPin = 11;
...
//------------------MAIN LOOP------------------------//
void loop() {
  data.joystick1xAxisBytes = map(analogRead(A2), 0, 1023, 0, 255); // Read Joystick 1 X-axis and map to bytes
  data.joystick1yAxisBytes = map(analogRead(A1), 0, 1023, 0, 255); // Read Joystick 1 Y-axis and map to bytes
  data.joystick2xAxisBytes = map(analogRead(A4), 0, 1023, 0, 255); // Read Joystick 2 X-axis and map to bytes
